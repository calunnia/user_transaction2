...
<br/>
  ...
  {
  loading ? 'Loading...'
          :  <><div className="users"><p>users = {users ? users.length : 0}</p>
                     {users.forEach(elem => <p>{elem.name}</p>)
                    }
               ...
               <input id="name"/>
               <button onClick={ () =>setDeposit( getAllDeposit(users,transactions,document.getElementById('name').value) ) }>Get All Deposit</button>
